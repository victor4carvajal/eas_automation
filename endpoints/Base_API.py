...
class Base_API:
    ...
    def get(self, url, headers={}):
        ...
        return {'status_code': response.status_code,'text':response.text,'json_response':json_response, 'error': error}


    def post(self, url,params=None, files= None, data=None,json=None,headers={}):
        "Post request"
        error = {}
        json_response = None
        try:
            response = self.request_obj.post(url,params=params,json=json,headers=headers,files=files,data=data)
            try:
                json_response = response.json()
            except:
                json_response = None
            ...
            print("Python says:%s" % str(e))
            json_response = None
        return {'url':url,'status_code': response.status_code,'text':response.text,'json_response':json_response, 'error': error}
